...
local player = game.Players.LocalPlayer or {Name = "Server"}
...
-- Tags meant for substitution
local customSubstitutionTags = {
    ["<player>"] = player.Name
}

-- Tags meant for styling
-- When writing the closing tag, make sure to list in reverse order as the opening tag
local customStyleTags = {
    ["<location>"] = {
        [[<font color="#42f581"><i><u>]],
        [[</u></i></font>]]
    }
}
...
function parseCustom(input)
	assert(type(input) == "string", "RichText: parseCustom input is not a string")
    
    -- substitution first
    local output = input
    for tag, substitute in pairs(customSubstitutionTags) do
        output = string.gsub(
            output, tag, substitute
        )
    end

    -- custom styles next
    for tag, info in pairs(customStyleTags) do
        output = string.gsub(
            output, tag, info[1]
        )
        local closeTag = "</" .. string.sub(tag, 2)
        output = string.gsub(
            output, closeTag, info[2]
        )
    end
